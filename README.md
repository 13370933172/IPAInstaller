...
从 `dist` 目录中获取 `ipa.exe`，放到任意文件夹即可使用。
...
在 `ipa.exe` 所在文件夹的地址栏中输入 `cmd` 然后回车：

```
C:\Users\xxx\Desktop\ipa> _
```
...
```bash
ipa.exe info C:\Users\xxx\Downloads\app.ipa
```
...
```bash
ipa.exe install C:\Users\xxx\Downloads\app.ipa
```

**多设备（指定 UDID）：**

```bash
ipa.exe install C:\Users\xxx\Downloads\app.ipa --udid 00008020-0015503826D2002E
```

**先卸载旧版本再安装：**

```bash
ipa.exe install C:\Users\xxx\Downloads\app.ipa --uninstall
```
...
```bash
ipa.exe log --output mylog.log
# 或使用短参数
ipa.exe log -o mylog.log
```
...
```bash
ipa.exe log --udid 00008020-0015503826D2002E
# 或使用短参数
ipa.exe log -u 00008020-0015503826D2002E
```

**组合使用：**

```bash
ipa.exe log -f "error" -o error.log -u 00008020-0015503826D2002E
```
...
| 命令 | 说明 |
|------|------|
| `ipa.exe list` | 列出所有已连接的 iOS 设备 |
| `ipa.exe info <ipa路径>` | 查看 IPA 包信息（不安装） |
| `ipa.exe install <ipa路径>` | 安装 IPA 到唯一连接的设备 |
| `ipa.exe install <ipa路径> -u <UDID>` | 安装到指定设备 |
| `ipa.exe install <ipa路径> -r` | 先卸载再安装 |
| `ipa.exe log` | 实时查看 iOS 设备系统日志 |
| `ipa.exe log -f <关键词>` | 过滤包含关键词的日志 |
| `ipa.exe log -o <文件路径>` | 将日志保存到指定文件 |
| `ipa.exe log -u <UDID>` | 查看指定设备的日志 |
| `ipa.exe --help` | 查看帮助信息 |
...
```batch
@echo off
ipa.exe install "C:\IPA\app1.ipa"
ipa.exe install "C:\IPA\app2.ipa"
ipa.exe install "C:\IPA\app3.ipa"
pause
```

### 6.3 发送到桌面快捷方式

右键 `ipa.exe` → 发送到 → 桌面快捷方式，方便快速使用。

### 6.4 配置环境变量（全局使用）

将 `ipa.exe` 所在目录添加到系统环境变量 `Path` 中，即可在任意位置通过 `cmd` 直接使用。
...
4. 点击 **新建**，填入 `ipa.exe` 所在的文件夹路径（例如 `C:\Tools\ipa`）
5. 依次点击 **确定** 保存所有窗口
...
```bash
ipa list                                              # 列出已连接的设备
ipa info C:\Users\xxx\Downloads\app.ipa               # 查看 IPA 包信息
ipa install C:\Users\xxx\Downloads\app.ipa            # 安装 IPA 到设备
ipa install C:\Users\xxx\Downloads\app.ipa -u <UDID>  # 安装到指定设备
ipa install C:\Users\xxx\Downloads\app.ipa -r         # 先卸载再安装
ipa log                                               # 实时查看系统日志
ipa log -f "MyApp"                                    # 过滤包含关键词的日志
ipa log -o mylog.log                                  # 将日志保存到文件
ipa log -u <UDID>                                     # 查看指定设备的日志
ipa --help                                            # 查看帮助信息
```
...
- **窗口 1**：执行安装命令
- **窗口 2**：运行 `ipa log -f "installd"` 实时监控安装进度
